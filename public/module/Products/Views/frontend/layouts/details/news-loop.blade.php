        ...
        <div class="header">
            @if($image_tag = get_image_tag($row->image_id,'full',['alt'=>$translation->title]))
                <header class="post-header">
                    <a href="{{route('news.detail', ['slug' => $translation->slug])}}">
                        {!! $image_tag !!}
                    </a>
                </header>
                <div class="cate">
            ...
            <div class="post-inner">
                <h3 class="post-title">
                    <a class="text-darken" href="{{route('news.detail', ['slug' => $translation->slug])}}"> {!! clean($translation->title) !!}</a>
                </h3>
                <div class="post-info">
                    <ul>
                ...
                <a class="btn-readmore" href="{{route('news.detail', ['slug' => $translation->slug])}}">{{ __('Read More')}}</a>
            </div>
        </div>
    </div>
@endforeach
