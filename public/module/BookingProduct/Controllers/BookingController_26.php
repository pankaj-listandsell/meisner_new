...
class BookingController extends Controller
{
    ...
    {
        $getProducts = Products::select('id','title','price','image_id')->where('cat_id',$request->id)->get();
        $viewRender = "";
    ...
    public function cartdata (Request $request)
    {
        ...
        foreach($request->cart_item as $val){
            ...
            $prodid = $getCart[0];
            $qty = $getCart[1];
            $getProducts = Products::select('id','title','price','image_id')->where('id',$prodid)->first();
        ...
        $additional_cost = 0;
        $gross_total_amount = $net_amount + $vatTax + $additional_cost;
        $summary = ['address'=>$request->address,'total_pieces'=>count($request->cart_item),'net_amount'=>$net_amount,'vat'=>$vatTax,'additional_cost'=>$additional_cost,'gross_total_amount'=>$gross_total_amount];
    ...
    public function search_category (Request $request)
    {
        ...
        if($request->type == 2){
            ...
            if(!empty($request->key)){
                $getProducts = Products::select('id','title','price','image_id')->where('cat_id',$request->category)->where('title','LIKE', '%' . $request->key . '%')->get();
