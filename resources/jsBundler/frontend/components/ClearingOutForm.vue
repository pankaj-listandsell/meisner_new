<template>
    <div>
            ...
            <div class="form-stepper">
                <div class="form-steps">
                    <div class="step" :class="{'step-active' : form.step === 1, 'step-done': form.step > 1}">
                        <span class="step-text">{{ trans('Request a free and inexpensive offer now') }} - {{ trans('Step 1 of 3') }}</span>
                    </div>
                    <div class="step" :class="{'step-active' : form.step === 2, 'step-done': form.step > 2}">
                        <span class="step-text">{{ trans('Just a few more questions') }} - {{ trans('Step 2 of 3') }}</span>
                    </div>
                    <div class="step" :class="{'step-active' : form.step === 3, 'step-done': form.step > 3}">
                        <span class="step-text">{{ trans('Your Contact Detail') }} - {{ trans('Step 3 of 3') }}</span>
                    </div>
                </div>
                ...
                <section v-show="form.step === 1">
                    ...
                    <div class="form-step-first">
                        ...
                        <div class="form-group">
                            <label for="prefer_date">{{ trans('Do you already have a preferred date?') }} <span
                                    class="sym-required">*</span></label>
            ...
            <transition name="slide-fade">
                <section v-show="form.step === 2">
                    ...
                    <div class="form-step-second">
                        ...
                        <div class="step-second-title clearfix">
                            <strong class="float-left">{{ trans('Clearing by') }}:</strong>
                        </div>
                        ...
                        <div class="form-group-inner">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <InputNumberField fieldName="order_living_space"
                                                    :schema="findFormField('order_living_space')"
                                                    @change="updateField('order_living_space', $event)"
                                                    :error="getError('order_living_space')"
                                                    :default-value="form.order_living_space"
                                        ></InputNumberField>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <InputNumberField fieldName="order_total_rooms"
                                                    :schema="findFormField('order_total_rooms')"
                                                    @change="updateField('order_total_rooms', $event)"
                                                    :error="getError('order_total_rooms')"
                                                    :default-value="form.order_total_rooms"
                                        ></InputNumberField>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="evacuation_site">
                                        {{ trans('Evacuation location') }}
                                        <span class="sym-required">*</span>
                                    </label>
                                    <SelectField fieldName="order_evacuation_site"
                                                 :schema="findFormField('order_evacuation_site')"
                                                 @change="updateField('order_evacuation_site', $event)"
                                                 :error="getError('order_evacuation_site')"
                                    ></SelectField>
                                </div>
                                <div class="form-group" v-if="form.order_evacuation_site && isBelongsToRelated('order_has_basement')">
                                    <label for="order_has_basement">
                                        {{ trans('Basement available') }}
                                    </label>
                                    <RelatedRadioField fieldName="order_has_basement"
                                                       :schema="findFormField('order_has_basement')"
                                                       @change="updateField('order_has_basement', $event)"
                                                       :error="getError('order_has_basement')"
                                    ></RelatedRadioField>
                                </div>
                                <div class="form-group" v-if="form.order_evacuation_site && isBelongsToRelated('order_has_more_storage')">
                                    <label for="order_has_more_storage">
                                        {{ trans('More storage space available') }}
                                    </label>
                                    <RelatedRadioField fieldName="order_has_more_storage"
                                                       :schema="findFormField('order_has_more_storage')"
                                                       @change="updateField('order_has_more_storage', $event)"
                                                       :error="getError('order_has_more_storage')"
                                    ></RelatedRadioField>
                                </div>
                            </div>
                            <div class="col-md-6">
                ...
                <section v-show="form.step === 3">
                    ...
                    <div class="form-step-third">
                        ...
                        <div class="form-group-inner">
                            ...
                            <div class="form-group">
                                <TextareaField fieldName="work_detail"
                                            :schema="findFormField('work_detail')"
                                            @change="updateField('work_detail', $event)"
                                            :error="getError('work_detail')"
                                               :default-value="form.work_detail"
                                ></TextareaField>
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="attachment">{{ trans('Upload plan, sketch or images (PDF/JPEG)') }}</label>
                            <FileField fieldName="attachment"
                                       :schema="findFormField('attachment')"
                                       @change="updateField('attachment', $event)"
        ...
        <template v-else>
            <div id="form_4a70a3722f" class="thank-you-message">
...
export default {
    ...
    methods: {
        ...
        registerForm() {
            ...
            if (!this.isValidInputs(this.getThirdStepFields())) {
                return;
            }
