...
(function ($) {
    ...
    $('.dungdt-btn-delete').click(function (e) {
        e.preventDefault();
        let $this = $(this);

        bookingCoreApp.showConfirm({
            message: $this.attr('data-confirm') ?? i18n.confirm_delete,
            callback: function(result){
                if(result){
                    let actionableForm = $('.actionable-form');
                    actionableForm.attr('action', $this.attr('data-action'));
                    actionableForm.find('[name=action]').val('delete');
                    let ids = '<input type="hidden" name="ids[]" value="'+$this.attr('data-id')+'">';
                    actionableForm.append(ids).submit();
                }
            }
        })
    });
    ...
    jQuery(function ($) {
        $('.has-datepicker').daterangepicker({
            ...
            locale:{
                format: bookingCore.bts_datepicker_format
            }
        }).on('apply.daterangepicker', function (ev, picker) {
            $(this).val(picker.startDate.format(bookingCore.bts_datepicker_format));
        });
    })
