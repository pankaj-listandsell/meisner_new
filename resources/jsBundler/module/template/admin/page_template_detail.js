...
export default function()
{
    ...
    window.manageBlocksScreen = new Vue({
        ...
        created(){
            ...
            this.title = current_template_title;
            this.templates = [...all_templates];
            $(document).on("template_changed", function(event, template_id) {
                this.onTemplateChange(template_id);
            }.bind(this));
        },
        mounted() {
            if (template_id == 0) {
        ...
        methods:{
            async onTemplateChange(template_id) {
                this.template_id = template_id;
                if (this.template_id > 0) {
                    await this.setBlockItem(this.getTemplateContentByTemplateId(template_id));
                    await this.reloadBlocks();
                } else {
                    this.initTinyMce();
                }
            },
            getTemplateContentByTemplateId(template_id) {
                ...
                ('save-db',
                    {
                        run: (editor, sender) =>
                        {
                            ...
                            let pageIdSelector = document.getElementById('page_id');

                            if (is_edit_page) {
                                $.ajax({
                                    url:bookingCore.admin_url+'/module/page/store-ajax',
                                    dataType:'json',
                                    type:'post',
                                    data:{
                                        page_id:pageIdSelector.value,
                                        content: htmldata,
                                        lang: current_menu_lang
                                    },
                                    success:function (res) {
