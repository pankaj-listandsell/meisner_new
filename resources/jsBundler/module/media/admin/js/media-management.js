    ...
    window.mediaManagement = new Vue({
        ...
        methods:{
            ...
            generateThumbnails() {
                var me = this;
                bookingCoreApp.showConfirm({
                    message: 'Do you want to generate thumbnails?',
                    callback: function(result){
                        if(result){
                            me.isLoading = true;
                            $.ajax({
                                url:bookingCore.admin_url+'/module/media/generate-thumbnails',
                                type:'POST',
                                data:{
                                    file_ids : me.selected
                                },
            ...
            deleteThumbnails() {
                var me = this;
                bookingCoreApp.showConfirm({
                    message: 'Do you want to delete only thumbnails?',
                    callback: function(result){
                        if(result){
                            me.isLoading = true;
                            $.ajax({
                                url:bookingCore.admin_url+'/module/media/delete-thumbnails',
                                type:'POST',
                                data:{
                                    file_ids : me.selected
                                },
            ...
            handleImageDblClick: function (file) {
                this.openModal(file.id, file.title, file.alt);
            },
            openModal: function (imageId, title, alt) {
                document.getElementById('imageTitle').value = title || '';
                document.getElementById('imageAlt').value = alt || '';
                document.getElementById('imageId').value = imageId;
                $('#mediaModal').modal('show');
            },
            saveMediaDetails: function () {
                ...
                fetch(bookingCore.admin_url+'/module/media/store_details', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const file = this.files.find(f => f.id === parseInt(imageId));
                        if (file) {
                            file.title = title;
                            file.alt = alt;
                        }
                ...
                .catch(error => {
                    ...
                    alert('An error occurred while updating media details.');
                });
            }
        }
    });
