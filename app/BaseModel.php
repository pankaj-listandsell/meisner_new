...
class BaseModel extends Model
{
    ...
    public function generateSlug($string = false, $count = 0)
    {
        ...
        $self = $this;
        $check = $model->where($this->slugField, $newSlug)
            ->where(function ($query) use ($self) {
                if (in_array('slug_affix', $self->getFillable())) {
                    $query->where('slug_affix', (string) $self->slug_affix);
                }
            })
            ->count();
...
//    }
    ...
    public function translation($locale = ''){
        return $this->hasOne($this->getTranslationModelNameDefault(),'origin_id')->where('locale', get_current_lang($locale));
    }
    public function translate($locale = false){
        return $this->translation($locale)->first();
    }
    ...
    public function translateOrOrigin($locale = false){
        ...
        if (empty($locale) || is_default_lang($locale)) {
    ...
    public function translateOrNewOrigin($locale = false){

        if (empty($locale) || is_default_lang($locale)) {
            $a = $this->getNewTranslation($locale);
            $a->fill(Arr::except($this->getAttributes(), ['id']));
        } else {
            $a = $this->translate($locale);
            if(!empty($a)) return $a;
            $a = $this->getNewTranslation($locale);
     ...
     * @return bool|null
     */
    public function saveOriginOrTranslation($locale = false, $saveSeo = true, $inputs = [])
    {
        if(!$locale or is_default_lang($locale) or empty(setting_item('site_enable_multi_lang'))){
            ...
            $translation = $this->translateOrNewOrigin($locale);
            if($translation){
                $translation->fill(count($inputs) > 0 ? $inputs : request()->input());
                $this->translation($locale)->save($translation);
                if($saveSeo){
