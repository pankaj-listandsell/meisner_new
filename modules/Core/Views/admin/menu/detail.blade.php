                ...
                <div v-for="(type,index) in item_types" class="panel panel-toggle-able">
                    ...
                    <div class="panel-body" v-show="type.open">
                        ...
                        <div class="list-scrollable" v-show="type.items.length">
                            <div v-for="item in type.items">
                                <label>
                                    <input v-model="type.selected"
                                           type="checkbox"
                            ...
                            <Draggable-Tree :value="items" draggable cross-tree :triggerClass="triggerClass">
                                <div slot-scope="{node,index, path, tree}" class="nestable-item-content" >
                                    ...
                                    <div class="menu-info" v-show="node._open">
                                        ...
                                        <template v-if="node.item_model=='custom'">
                                            <div class="form-group">
                                                <label>{{__('URL')}}</label>
                                                <input type="text" v-model="node.url" class="form-control input-sm">
                                            </div>
                                            <div class="form-group">
                                                <label>{{__('Show content')}}</label>
                                                <label>
                                                    <input type="radio" v-model="node.has_content" :value="1"/>
                                                    <span>Yes</span>
                                                </label>
                                                <label>
                                                    <input type="radio" v-model="node.has_content" :value="0"/>
                                                    <span>No</span>
                                                </label>
                                            </div>
                                            <div class="form-group" v-if="node.has_content">
                                                <label>{{__('Content')}}</label>
                                                <vue2-tinymce-editor v-model="node.content"></vue2-tinymce-editor>
                                            </div>
                                        </template>
                                        <div class="row">
                                            <div class="col-md-6">
