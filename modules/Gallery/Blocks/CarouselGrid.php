...
class CarouselGrid extends BaseBlock
{
        ...
        $arg[] = [
            ...
            'settings' => [
                [
                    ...
                    'select2'   => [
                        'ajax'     => [
                            'url'      => route('gallery.admin.getForSelect2', ["type" => "carousel"]),
                            'dataType' => 'json'
                        ],
                        'width'    => '100%',
                        'multiple' => "true",
    ...
    public function content($model = []){
        ...
        return view('Gallery::frontend.carousel-grid', $data);
    }
    ...
    public function query($model){
        ...
        if(!empty( $model['carousel_ids'] )){
            $model_Gallery->whereIn("bravo_gallery.id",$model['carousel_ids']);
        }
        $model_Gallery->orderBy("bravo_gallery.".$model['order'], $model['order_by']);
        $model_Gallery->where("bravo_gallery.status", "publish");
        $model_Gallery->where("bravo_gallery.type", "carousel");
