...
class UserController extends AdminController
{
    ...
    public function password(Request $request,$id){
        ...
        $data  = [
            ...
            'page_title' => __("Change User Password"),
            'breadcrumbs'=>[
                [
                    'name'=>__("Users"),
                    'url'=>route('user.admin.index')
    ...
    public function store(Request $request, $id)
    {
        ...
        $row->active_status = $request->input('active_status');
    ...
    public function getRolesForSelect2(Request $request)
    {
        ...
        if ($pre_selected && $selected) {
            if (is_array($selected)) {
                $res = \Modules\User\Models\Role::whereIn('id', $selected)->take(50)->get();
                $items = [];
                if(!empty($res)){
                    foreach ($res as $item){
                        $items[] = [
                ...
                $item = \Modules\User\Models\Role::find($selected);
                if(!empty($item)){
                    return $this->sendSuccess([
                        'text' => $item->getDisplayName() . ' (#' . $item->id . ')',
                    ]);
        ...
        $q = $request->query('q');
        $query = \Modules\User\Models\Role::select('*');
        if ($q) {
            $query->where(function ($query) use ($q) {
                $query->where('name', 'like', '%' . $q . '%')->orWhere('id', $q);
            });
        }
        $res = $query->orderBy('name', 'asc')->limit(100)->get();
        $data = [];
        if (!empty($res)) {
            foreach ($res as $item) {
                $data[] = [
