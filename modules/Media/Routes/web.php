...
Route::group(['middleware' => ['auth'],'prefix' => config('admin.admin_route_prefix')],function(){
    ...
    Route::post('/module/media/generate-thumbnails','\Modules\Media\Admin\MediaController@generateThumbnails');
    Route::post('/module/media/delete-thumbnails','\Modules\Media\Admin\MediaController@deleteThumbnails');

    Route::post('/media/folder','FolderController@index')->name('admin.media.folder.index');

    Route::post('/module/media/store_details','\Modules\Media\Admin\MediaController@store_details');
});
