...
class MediaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // generateThumbnail(getAbsoluteImagePath($check));
    ...
    public function store_details(Request $request)
    {
        ...
        $media = MediaFile::find($validatedData['id']);
        $media->title = $validatedData['title'];
        $media->alt = $validatedData['alt'];
        $media->save();
        ...
        if(!empty($files->count())){
            foreach ($files as $file){
                if($storage->exists($file->file_path)){
                    if ($file->has_thumb) {
                        deleteThumbnail(public_path('uploads/'.$file->file_path));
                    }
    ...
    public function generateThumbnails(Request $request)
    {
        ...
        if(!empty($files->count())){
            foreach ($files as $file){
                if($storage->exists($file->file_path)){
                    generateThumbnail(public_path('uploads/'.$file->file_path));
                    $file->has_thumb = 1;
                    $file->save();
                }
            }
            return $this->sendSuccess([],__("Generated thumbnail successfully"));
        }
        return $this->sendError(__("File not found!"));
    }

    public function deleteThumbnails(Request $request)
    {
        if(is_demo_mode()){
            return $this->sendError(__("Can not remove!"));
        }
