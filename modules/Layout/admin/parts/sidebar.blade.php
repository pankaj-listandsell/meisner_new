    ...
    foreach ($menus as $k => $menuItem) {
        ...
        if (isset($menuItem['children']) && count($menuItem['children']) > 0) {
            foreach ($menuItem['children'] as $subMenu) {
                if (isset($subMenu['routes'])) {
                    if (in_array($currentRoute ,$subMenu['routes'])) {
            ...
            if ($currentUrl == url($menuItem['url'])) {
                $menus[$k]['class'] = 'active';
            }
        }
        if (!empty($menuItem['children'])) {
            ...
            foreach ($menuItem['children'] as $k2 => $menuItem2) {
                ...
                if (isset($menuItem2['routes'])) {
                    if (in_array($currentRoute, $menuItem2['routes'])) {
                        $menus[$k]['children'][$k2]['class'] = 'active';
                    }
                } else {
                    if ($currentUrl == url($menuItem2['url'])) {
                        $menus[$k]['children'][$k2]['class'] = 'active';
                    }
                }
            ...
            @if(!empty($menuItem['children']))
                ...
                <ul class="children">
                    @foreach($menuItem['children'] as $menuItem2)
                        <li class="{{ isset($menuItem2['class']) ? $menuItem2['class'] : '' }}"><a href="{{ url($menuItem2['url']) }}">
